...
ControlBase * WalkCalibration::activate(MotionManager::MC_ID disp_id, Socket * gui) {
...
void WalkCalibration::addSample() {
	...
	case fs:
		if(isPolar) {
			dx=firstIn*cos(secondIn*M_PI/180)*10/dt;
			dy=firstIn*sin(secondIn*M_PI/180)*10/dt;
		} else {
			dx=firstIn*10/dt;
			dy=secondIn*10/dt;
		}
	...
	case fr:
		...
		da=(old_a>0?secondIn:-secondIn)*M_PI/180/dt;
		addSample(data[curType], dx,dy,da);
		report(0,old_x,dx);
		report(2,old_a,da);
		break;
